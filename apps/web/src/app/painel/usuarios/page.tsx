...
export default function UsuariosPage() {
  ...
  const criar = useMutation({
    ...
    onSuccess: () => { setNovo(false); setForm({ nome: '', cpf: '', senha: '', permissoes: [], rotaIds: [] }); qc.invalidateQueries({ queryKey: ['usuarios'] }); },
  ...
  const salvarPermissoes = useMutation({
    ...
    onSuccess: () => { setEditandoPerm(null); qc.invalidateQueries({ queryKey: ['usuarios'] }); },
    onError: (e: Error) => setErro(e.message),
  });
  ...
  return (
    <div className="mx-auto max-w-5xl p-8">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-feltro">Usuários</h1>
        ...
        <div className="mb-6 rounded-xl bg-white p-5 shadow-sm">
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            {rotas?.map((r) => (
              <button key={r.id} onClick={() => setForm({ ...form, rotaIds: alternar(form.rotaIds, r.id) })}
                className={`rounded-full border px-3 py-1 text-xs ${form.rotaIds.includes(r.id) ? 'border-feltro bg-feltro text-white' : 'border-stone-300'}`}>
                {r.nome}
              </button>
            ))}
          </div>
          ...
          {grupos.map((g) => (
            <div key={g} className="mb-3">
              <p className="mb-1 text-xs uppercase tracking-wide text-stone-400">{g}</p>
              <div className="flex flex-wrap gap-2">
                {permissoes?.filter((p) => p.grupo === g).map((p) => (
                  <button key={p.chave} title={p.descricao}
                    ...
                    className={`rounded-full border px-3 py-1 text-xs ${form.permissoes.includes(p.chave) ? 'border-feltro bg-feltro text-white' : 'border-stone-300'}`}>
                    {p.chave}
                  </button>
                ))}
              </div>
      ...
      {editandoPerm && (
        <div className="mb-6 rounded-xl border-2 border-feltro/30 bg-white p-5 shadow-sm">
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            {rotas?.map((r) => (
              <button key={r.id} onClick={() => setRotasEdit(alternar(rotasEdit, r.id))}
                className={`rounded-full border px-3 py-1 text-xs ${rotasEdit.includes(r.id) ? 'border-feltro bg-feltro text-white' : 'border-stone-300'}`}>
                {r.nome}
              </button>
            ))}
          </div>
              ...
              <div className="flex flex-wrap gap-2">
                {permissoes?.filter((p) => p.grupo === g).map((p) => (
                  <button key={p.chave} title={p.descricao}
                    ...
                    className={`rounded-full border px-3 py-1 text-xs ${permEdit.includes(p.chave) ? 'border-feltro bg-feltro text-white' : 'border-stone-300'}`}>
                    {p.chave}
                  </button>
                ))}
              </div>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setEditandoPerm(null)} className="rounded-lg border px-4 py-2 text-sm text-stone-500">
              Cancelar
            </button>
          </div>
        </div>
      ...
      {erro && !novo && <p className="mb-3 text-sm text-red-600">{erro}</p>}
      ...
      <div className="overflow-hidden rounded-xl bg-white shadow-sm">
        <table className="w-full text-sm">
          <thead><tr className="border-b bg-stone-50 text-left text-xs uppercase text-stone-400">
            <th className="p-3">Nome</th><th>CPF</th><th>Rotas</th><th>Permissões</th><th>Status</th><th></th>
          </tr></thead>
          <tbody>
            {usuarios?.map((u) => (
              <tr key={u.id} className="border-b last:border-0">
                <td className="p-3 font-medium">{u.nome}</td>
                <td className="text-stone-500">{u.cpf}</td>
                <td className="text-xs">{u.rotas.map((r) => r.nome).join(', ') || '—'}</td>
                <td className="text-xs text-stone-500">{u.permissoes.length} permissão(ões)</td>
                <td>{u.ativo ? <span className="text-feltro">Ativo</span> : <span className="text-red-500">Inativo</span>}</td>
                <td className="pr-3 text-right text-xs whitespace-nowrap">
                  ...
                  <button className="text-stone-400 hover:text-feltro" onClick={() => resetarSenha(u)}>
                    resetar senha
                  </button>
                  <span className="mx-1 text-stone-300">·</span>
                  <button className="text-stone-400 hover:text-feltro" onClick={() => abrirPermissoes(u)}>
                    permissões
                  </button>
                </td>
              </tr>
