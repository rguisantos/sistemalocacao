  ...
  return (
    <section className="rounded-xl bg-white p-5 shadow-sm">
      ...
      <div className="flex flex-wrap gap-2">
        {itens?.map((i) => (
          <span key={i.id} className="inline-flex items-center gap-1 rounded-full bg-stone-100 px-3 py-1 text-sm">
            {i[campo]}
            <button onClick={() => remover.mutate(i.id)} className="text-stone-400 hover:text-red-500">×</button>
          </span>
        ))}
        {(itens?.length ?? 0) === 0 && <span className="text-sm text-stone-400">Nenhum cadastrado.</span>}
      </div>
    </section>
  );
}
...
export default function CadastrosPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl gap-6 p-8 lg:grid lg:grid-cols-2">
      <section className="mb-6 rounded-xl bg-white p-5 shadow-sm lg:mb-0">
        ...
        {rotas?.map((r) => (
          <div key={r.id} className="flex justify-between border-b py-2 text-sm last:border-0">
            <span>{r.nome}</span><span className="text-stone-400">{r._count.clientes} cliente(s)</span>
          </div>
        ))}
      </section>
      ...
      <section className="rounded-xl bg-white p-5 shadow-sm">
        ...
        {depositos?.map((d) => (
          <div key={d.id} className="flex justify-between border-b py-2 text-sm last:border-0">
            <span>{d.nome}</span><span className="text-stone-400">{d.cidade ?? ''}</span>
          </div>
        ))}
      </section>
