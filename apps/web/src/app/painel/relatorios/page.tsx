...
export default function RelatoriosPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl p-8">
      <h1 className="mb-2 text-2xl font-bold text-feltro print:text-black">Relatórios</h1>

      <div className="mb-4 flex flex-wrap gap-2 print:hidden">
        {PRESETS.map(([v, r]) => (
          <button key={v} onClick={() => setDimensao(v)}
            className={`rounded-full border px-4 py-1.5 text-sm ${dimensao === v ? 'border-feltro bg-feltro text-white' : 'border-stone-300 bg-white'}`}>
            {r}
          </button>
        ))}
      </div>
      ...
      <p className="mb-3 hidden text-sm text-stone-500 print:block">
      ...
      <div className="overflow-hidden rounded-xl bg-white shadow-sm print:shadow-none">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-stone-50 text-left text-xs uppercase text-stone-400 print:bg-white">
        ...
        {isFetching && <p className="p-4 text-center text-stone-400">Carregando…</p>}
        {!isFetching && linhas.length === 0 && (
          <p className="p-6 text-center text-stone-400">Sem cobranças no período selecionado.</p>
        )}
      </div>
    </div>
  );
