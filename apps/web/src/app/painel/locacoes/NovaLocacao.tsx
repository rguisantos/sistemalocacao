  ...
  return (
    <div className="mb-6 rounded-xl bg-white p-5 shadow-sm">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {([
          ...
          <button key={valor} onClick={() => setRegra(valor)}
            className={`rounded-full border px-4 py-1.5 text-sm ${regra === valor ? 'border-feltro bg-feltro text-white' : 'border-stone-300'}`}>
            {rotulo}
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button onClick={fechar} className="rounded-lg border border-stone-300 px-4 py-2 text-sm text-stone-600">
          Cancelar
        </button>
      </div>
    </div>
