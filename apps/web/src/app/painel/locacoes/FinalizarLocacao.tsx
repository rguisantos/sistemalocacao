...
export function FinalizarLocacao({
  ...
  const finalizar = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ titulo: 'Locação finalizada', descricao: tipo === 'RELOCACAO' ? 'Produto liberado para relocação.' : 'Produto recolhido ao depósito.' });
      if (tipo === 'RELOCACAO') {
  ...
  return (
    <div className="border-t bg-amber-500/10/60 p-4">
      <p className="mb-3 text-sm font-semibold text-foreground/90">Finalizar locação de {plaqueta}</p>
        ...
        {([['DEPOSITO', 'Recolher para depósito'], ['RELOCACAO', 'Relocação (novo cliente/endereço)']] as const).map(
          ([valor, rotulo]) => (
            <button key={valor} onClick={() => setTipo(valor)}
              className={`rounded-lg border px-3 py-1.5 text-sm ${tipo === valor ? 'border-feltro bg-feltro text-white' : 'border-border bg-card'}`}>
              {rotulo}
            </button>
          )
        )}
      ...
      <div className="flex gap-2">
        ...
        <button onClick={fechar} className="rounded-lg border border-border bg-card px-4 py-2 text-sm text-foreground/80">
          Cancelar
        </button>
      </div>
    </div>
