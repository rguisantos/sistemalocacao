...
export function EditarLocacao({ locacao, fechar }: Props) {
  ...
  return (
    <div className="border-t bg-muted/50 p-4">
      <p className="mb-1 text-sm font-semibold text-foreground/90">Editar regras da locação</p>
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        {([['VALOR_FIXO', 'Valor fixo'], ['PERCENTUAL_A_RECEBER', '% a receber'], ['PERCENTUAL_A_PAGAR', '% a pagar']] as const).map(([v, r]) => (
          <button key={v} onClick={() => setRegra(v)}
            className={`rounded-full border px-3 py-1.5 text-sm ${regra === v ? 'border-feltro bg-feltro text-white' : 'border-border bg-card'}`}>
            {r}
          </button>
        ))}
      </div>
